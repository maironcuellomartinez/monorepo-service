...
import { RequestType, RequestTypeUtils, ResponseServiceNowSuccess, TracingClient } from '../../common';
 ...
 */
const SN_CLOSED_STATES = new Set(['resolved', 'closed', '0', '3', '4', '6', '7']);
...
export class ServiceNowClientService {
     ...
     */
    async postToServiceNow(type: RequestType, payload: Record<string, any>): Promise<ResponseServiceNowSuccess> {
        return this.tracing.run(
            'servicenow.post',
            { kind: 'client', attributes: { 'snow.type': type, 'snow.baseUrl': process.env.BASE_URL_SERVICENOW ?? '' } },
            async () => {
                const baseUrl = process.env.BASE_URL_SERVICENOW ?? '';
                const endpoint = `${baseUrl}${RequestTypeUtils.getEndpoint(type)}`;
                const correlationId = uuidv4();

                this.logger.debug(`[${type}] Enviando solicitud con correlationId: ${correlationId}`);
                ...
                this.logger.log(`[${type}] Respuesta recibida → sys_id: ${result.result.sys_id} | number: ${result.result.number}`);
                return result;
            },
        );
    }
    ...
    async patchToServiceNow(type: RequestType, sysId: string, body: Record<string, any>): Promise<void> {
        ...
        try {
            ...
            this.logger.log(`[${type}] PATCH OK → sys_id=${sysId} campos=${Object.keys(body).join(',')}`);
        } catch (error: any) {
            this.logger.error(`[${type}] Error en PATCH ServiceNow: ${error.message}`);
            throw this.errorFactory.create(
     ...
     */
    async getTicketState(type: RequestType, sysId: string): Promise<{ state: string; closed: boolean } | null> {
        const baseUrl = process.env.BASE_URL_SERVICENOW ?? '';
        const endpoint = `${baseUrl}${RequestTypeUtils.getEndpoint(type)}/${sysId}`;
