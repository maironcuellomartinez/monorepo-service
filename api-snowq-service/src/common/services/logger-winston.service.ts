...
export class LoggerService implements NestLoggerService, OnModuleDestroy {
                ...
                new transports.Console({
                    format: format.combine(
                        ...
                        format.printf(info => {
                            ...
                            const id = correlationId ?? this.correlationService.getCorrelationId() ?? 'no-cid';
                            let line = `[${timestamp}] [${id}] ${level}: [${context}] ${message}`;
    ...
    log(message: string, context: string = 'Application', meta?: Record<string, any>) {
        this.logger.info(message, { context, ...this.getCommonMeta(), ...meta });
    }

    error(message: string, trace: string, context: string = 'Application', meta?: Record<string, any>) {
        this.logger.error(message, { context, stack: trace, ...this.getCommonMeta(), ...meta });
    }

    warn(message: string, context: string = 'Application', meta?: Record<string, any>) {
        this.logger.warn(message, { context, ...this.getCommonMeta(), ...meta });
    }

    debug(message: string, context: string = 'Application', meta?: Record<string, any>) {
        this.logger.debug(message, { context, ...this.getCommonMeta(), ...meta });
    }

    verbose(message: string, context: string = 'Application', meta?: Record<string, any>) {
