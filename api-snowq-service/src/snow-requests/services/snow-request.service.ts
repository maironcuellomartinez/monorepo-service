...
export class SnowRequestService {
  ...
  async create(data: Partial<SnowRequestEntity>): Promise<SnowRequestEntity> {
    const maxRetries =
      data.maxRetries ??
      RequestPriorityUtils.getMaxRetries(data.priority as RequestPriority);
  ...
  ): Promise<SnowRequestEntity | null> {
   ...
   * Solo devuelve registros cuyo nextRetryAt ya venció o es nulo, y cuyo
   * expiresAt no haya vencido (evita despachar alertas ya expiradas en la
   * ventana de hasta 30s entre pasadas de checkExpiry()).
   */
  async findPendingQueue(limit: number): Promise<SnowRequestEntity[]> {
    ...
    return this.repo
      ...
      .andWhere('(r.nextRetryAt IS NULL OR r.nextRetryAt <= :now)', { now })
  ...
  async markAsRetry(
    ...
    const maxRetries =
      entity.maxRetries ?? RequestPriorityUtils.getMaxRetries(entity.priority);
    ...
    if (newRetryCount >= maxRetries) {
      ...
      const delayMs =
        retryAfterMs ??
        RequestPriorityUtils.getRetryDelay(entity.priority, entity.retryCount);
      await this.repo.update(
        { correlationId },
        {
   ...
   */
  async cancelByFingerprint(fingerprint: string): Promise<{
    action: 'CANCELLED' | 'TOO_LATE' | 'RESOLVED';
    entity: SnowRequestEntity;
  } | null> {
    // 1. QUEUED → cancelar. El update queda condicionado a status=QUEUED
    // para no pisar un registro que el worker recién marcó IN_PROGRESS entre
    // el findOne y este update (el poll corre cada 500ms, la ventana es real).
    const queued = await this.repo.findOne({
      where: { fingerprint, status: STATUS.QUEUED },
    });
    if (queued) {
      const result = await this.repo.update(
        { correlationId: queued.correlationId, status: STATUS.QUEUED },
        { status: STATUS.CANCELLED },
      );
      if ((result.affected ?? 0) > 0) {
        return {
          action: 'CANCELLED',
          entity: { ...queued, status: STATUS.CANCELLED },
        };
      }
      // Perdió la carrera contra el worker — el registro ya no está QUEUED.
      // Cae al chequeo de IN_PROGRESS de abajo en vez de reportar CANCELLED
      // sobre un ticket que en realidad ya se está enviando a ServiceNow.
    }

    // 2. IN_PROGRESS → demasiado tarde
    const inProgress = await this.repo.findOne({
      where: { fingerprint, status: STATUS.IN_PROGRESS },
    });
    if (inProgress) {
    ...
    const processed = await this.repo
      .createQueryBuilder('r')
      .where('r.fingerprint = :fingerprint', { fingerprint })
    ...
    this.logger.debug(
      `[CANCELBYFINGERPRINT] fingerprint=${fingerprint.substring(0, 12)}... — ${all.length} ticket(s) encontrado(s): ` +
        all
          .map(
            (t) =>
              `[${t.internalNumber} status=${t.status} resolvedAt=${t.resolvedAt ?? 'NULL'}]`,
          )
          .join(', '),
    );
  ...
  async retryFailed(correlationId: string): Promise<SnowRequestEntity | null> {
    const entity = await this.repo.findOne({
      where: { correlationId, status: STATUS.FAILED },
    });
   ...
   *
   * Los `immediate=true` quedan excluidos de ese reintento: el caller original
   * de un request inmediato ya recibió (o va a recibir) un error de esa llamada
   * y puede haber disparado un fallback asíncrono por su cuenta. Reencolarlos
   * acá arriesga crear un segundo ticket duplicado en ServiceNow para el mismo
   * incidente — se marcan FAILED directamente, sin reintento automático.
   */
  async recoverStuckProcessing(): Promise<void> {
    ...
    await this.repo.update(
      ...
      },
      {
        status: STATUS.FAILED,
        lastError:
          'Proceso reiniciado con request immediate en vuelo — no se reintenta automaticamente para evitar duplicar el ticket en ServiceNow',
      },
    );
  ...
  async retryFiltered(filter: DlqFilterDto): Promise<number> {
    ...
    if (filter.source)
      qb.andWhere('source = :source', { source: filter.source });
    if (filter.since)
      qb.andWhere('updatedAt >= :since', { since: new Date(filter.since) });
    if (filter.until)
      qb.andWhere('updatedAt <= :until', { until: new Date(filter.until) });
  ...
  async getDlqStats(): Promise<{
    ...
    for (const r of failed) {
      ...
      const priorityLabel =
        ['', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL'][r.priority] ??
        String(r.priority);
