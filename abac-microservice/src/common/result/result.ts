...
export class Result<T, E = Error> {
    ...
    /**
     * Constructor privado para crear instancias de Result.
     * @param isSuccess Indica si el resultado es exitoso.
     * @param value Valor en caso de éxito.
     * @param error Error en caso de fallo.
     * @returns Instancia de Result.
     * @description Este constructor es privado para asegurar que las instancias de Result solo se creen a través de los 
     * métodos estáticos `ok` y `err`. Se usa para crear instancias de Result. Object.freeze(this) se usa para asegurar que el objeto sea inmutable.
     * @example
     * const r = new Result(true, 42, undefined);
     * const r2 = new Result(false, undefined, new Error('algo falló'));
     */
    ...
    /**
     * Obtiene el error interno. Lanza un error si el Result es failure.
     * @throws Error
     */
     ...
     * @example
     * const value = !result.unwrap().expect('No se pudo obtener el usuario');
     */
     ...
     * result.map(x => x * 2);
     * @example
     * const value = result.map(x => x * 2).unwrap();
     */
     ...
     * @param fn Función efecto
     * @example
     * result.tapError(err => console.error(err));
     */
...
/**
 * Desempaqueta una tupla de Results en una tupla de valores.
 */
type UnwrapTuple<T extends any[], E> = {
    [K in keyof T]: T[K];
};
 ...
 * @example
 * const combined = combineMultipleTuple(
 *   Result.ok(1),
 *   Result.ok(2),
 *   Result.ok(3)
 * );
...
export function combineMultipleTuple<T extends any[], E>(
    ...
    return Result.ok(values as UnwrapTuple<T, E>);
}

/**
 * Versión asíncrona de combine.
 * @example
 * const combined = combinePromises([
 ...
 * results.unwrap() // [10, 5, 3]
 * @example
 * const users = [{id: 1}, {id: 2}, {id: 3}];
 * const results = traverse(users, u => fetchUser(u.id));
 * results.unwrap() // [User, User, User]
 * results.unwrapError() // [Error, Error, Error]
 */
...
/**
 * Tipo Result con error por defecto Error.
 * @template T Tipo del valor en éxito
 */
type ResultAlias<T, E = Error> = Result<T, E>;
