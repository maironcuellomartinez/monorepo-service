...
export class InvalidDateRangeError extends DomainError {
    ...
    constructor(start: Date, end: Date) {
        super(`Invalid date range: ${start.toISOString()} must be before ${end.toISOString()}`);
    }
}
...
export class InsufficientSlotsError extends DomainError {
    ...
    constructor(requestedMinutes: number, availableMinutes: number) {
        super(`Not enough consecutive slots available: need ${requestedMinutes} minutes, only ${availableMinutes} available`);
    }
}
