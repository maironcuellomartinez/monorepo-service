...
export class Incident {
  ...
  get servicenowId(): ServiceNowId | null {
    return this._servicenowId;
  }
  get servicenowNumber(): ServiceNowNumber | null {
    return this._servicenowNumber;
  }
  ...
  get snowqCorrelationId(): string | null {
    return this._snowqCorrelationId;
  }
  ...
  isCreated(): boolean {
    return (
      this._status === IncidentStatus.CREATED &&
      this._scheduledRange.start > new Date()
    );
  }
  ...
  deliver(technicianId: TechnicianId): Result<void> {
    ...
    this.addEvent(
      new DomainEvent('INCIDENT_DELIVERED', this._id, 'Incident', {
        technicianId,
        previousStatus: IncidentStatus.CREATED,
      }),
    );
  ...
  take(technicianId: TechnicianId): Result<void> {
    ...
    this.addEvent(
      new DomainEvent('INCIDENT_TAKEN', this._id, 'Incident', {
        technicianId,
  ...
  release(technicianId: TechnicianId, reason?: string): Result<void> {
    ...
    this.addEvent(
      new DomainEvent('INCIDENT_RELEASED', this._id, 'Incident', {
        technicianId,
        reason,
        previousTechnicianId,
      }),
  ...
  validate(): Result<void> {
    ...
    this.addEvent(new DomainEvent('INCIDENT_VALIDATED', this._id, 'Incident'));
  ...
  reopen(reason?: string): Result<void> {
    ...
    this.addEvent(
      new DomainEvent('INCIDENT_REOPENED', this._id, 'Incident', {
        reason,
      }),
    );
  ...
  updateServiceNowInfo(
    ...
    this.addEvent(
      new DomainEvent('INCIDENT_SERVICENOW_UPDATED', this._id, 'Incident', {
        servicenowId: servicenowId.value,
        servicenowNumber: servicenowNumber.value,
      }),
    );
  ...
  changeStatus(
    ...
    const validTransitions =
      (
        INCIDENT_CONSTANTS.VALID_STATUS_TRANSITIONS as Record<
          string,
          readonly string[]
        >
      )[this._status] ?? [];
    ...
    this.addEvent(
      new DomainEvent('INCIDENT_STATUS_CHANGED', this._id, 'Incident', {
        technicianId,
        oldStatus,
        newStatus,
        comment,
  ...
  closeFromExternalSync(comment?: string): Result<void> {
    if (
      ...
      TERMINAL_STATUSES.includes(this._status)
    ) {
      return Result.err(
        new InvalidIncidentStateError(
          this._status,
          'closeFromExternalSync (ya es terminal)',
        ),
      );
    }
  ...
  addComment(technicianId: TechnicianId, comment: string): Result<void> {
    ...
    if (comment.length > INCIDENT_CONSTANTS.MAX_COMMENT_LENGTH) {
      return Result.err(
        new Error(
          `Comment cannot exceed ${INCIDENT_CONSTANTS.MAX_COMMENT_LENGTH} characters`,
        ),
      );
    }
    ...
    this.addEvent(
      new DomainEvent('INCIDENT_COMMENT_ADDED', this._id, 'Incident', {
        technicianId,
        comment,
      }),
    );
  ...
  assignLocker(lockerId: LockerId): Result<void> {
    ...
    if (terminalStatuses.includes(this._status)) {
      return Result.err(
        new InvalidIncidentStateError(this._status, 'assign locker'),
      );
    }
    ...
    this.addEvent(
      new DomainEvent('INCIDENT_LOCKER_ASSIGNED', this._id, 'Incident', {
  ...
  releaseLocker(): Result<void> {
    ...
    this.addEvent(
      new DomainEvent('INCIDENT_LOCKER_RELEASED', this._id, 'Incident', {
  ...
  static create(
    ...
    if (
      duration < INCIDENT_CONSTANTS.MIN_DURATION_MINUTES ||
      duration > INCIDENT_CONSTANTS.MAX_DURATION_MINUTES
    ) {
      return Result.err(
        new Error(
          `Duration must be between ${INCIDENT_CONSTANTS.MIN_DURATION_MINUTES} and ${INCIDENT_CONSTANTS.MAX_DURATION_MINUTES} minutes`,
        ),
      );
    }
    ...
    incident.addEvent(
      new DomainEvent('INCIDENT_CREATED', id, 'Incident', {
        issueTypeId,
        customerId,
        cornerId,
        slotIds,
      ...
      switch (event.type) {
        case 'INCIDENT_CREATED': {
          ...
          if (createResult.isFailure)
            return Result.err(createResult.unwrapError());
        ...
        case 'INCIDENT_VALIDATED':
          if (incident)
            incident.replayStatusChanged({
              ...event,
              data: { newStatus: IncidentStatus.VALIDATED },
            });
          break;
