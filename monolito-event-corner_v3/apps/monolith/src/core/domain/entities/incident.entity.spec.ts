...
function makeIncident(overrides?: { status?: IncidentStatus }): Incident {
  ...
  if (target === IncidentStatus.VALIDATED) {
    incident.validate();
    return incident;
  }
...
describe('Incident.take()', () => {
  ...
  it('funciona desde estado PENDING_THIRD_PARTY', () => {
    const incident = makeIncident({
      status: IncidentStatus.PENDING_THIRD_PARTY,
    });
...
describe('Incident.changeStatus()', () => {
  ...
  it('IN_PROGRESS → CLOSED (cierre directo) registra closedAt y borra técnico', () => {
    const incident = makeIncident({ status: IncidentStatus.IN_PROGRESS });

    const result = incident.changeStatus(IncidentStatus.CLOSED, TECH);
  ...
  it('IN_PROGRESS → CANCELED (cancelación directa) borra técnico', () => {
    const incident = makeIncident({ status: IncidentStatus.IN_PROGRESS });

    const result = incident.changeStatus(IncidentStatus.CANCELED, TECH);
...
describe('Incident.reopen()', () => {
  ...
  it('transiciona de CANCELED a REOPENED y borra técnico', () => {
    const incident = makeIncident({ status: IncidentStatus.CANCELED });
    const result = incident.reopen('se retoma la incidencia');
    expect(result.isSuccess).toBe(true);
    expect(incident.status).toBe(IncidentStatus.REOPENED);
    expect(incident.currentTechnicianId).toBeNull();
  });
