...
import { IAvailabilityService, AvailabilityQuery, SlotAvailabilityDto, TechnicianAvailabilityDto } from '../../ports/incoming/availability/availability-service.port';
...
export class AvailabilityService implements IAvailabilityService {
    ...
    async getAvailability(query: AvailabilityQuery): Promise<Result<SlotAvailabilityDto[]>> {
    ...
    async getTechniciansAvailability(
        ...
        const techResult = await this.technicianRepo.findByCorner(cornerId as unknown as CornerId);
        ...
        const cornerResult = await this.cornerRepo.findById(cornerId as unknown as CornerId);
        ...
        const activeIncidents = incidentResult.unwrap().filter(i =>
    ...
    async checkSlotAvailability(
        ...
        const slotsResult = await this.slotRepo.findConsecutiveSlots(cornerId, startTime, duration);
    ...
    private async calculateAvailability(query: AvailabilityQuery): Promise<SlotAvailabilityDto[] | Error> {
        // 1. Validar que el corner existe y obtener su duración de slot
        const cornerResult = await this.cornerRepo.findById(query.cornerId as unknown as CornerId);
        ...
        const slotsResult = await this.slotRepo.findByCornerAndDate(query.cornerId, query.date);
        ...
        const firstSlotDurationMin = (activeSlots[0].timeRange.end.getTime() - activeSlots[0].timeRange.start.getTime()) / 60_000;
        ...
        const techResult = await this.technicianRepo.findByCorner(query.cornerId as unknown as CornerId);
        ...
        const incidents = incidentResult.unwrap().filter(i =>
            i.status !== IncidentStatus.CANCELED && i.status !== IncidentStatus.CLOSED,
        );
        ...
        for (let i = 0; i <= activeSlots.length - slotsNeeded; i++) {
            ...
            if (!this.areConsecutive(window, firstSlotDurationMin)) continue;
            ...
            const heldByCurrentUser = !!userId && window.every(s => s.isHeldBy(userId));
            ...
            result.push({
                ...
                occupiedSlots: allUsable ? [] : window
                    .filter(s => !s.isAvailableForUser(userId))
                    .map(s => s.id.toString()),
    ...
    private areConsecutive(slots: Slot[], slotDurationMin: number): boolean {
        for (let i = 1; i < slots.length; i++) {
            const expectedStart = new Date(
                slots[i - 1].timeRange.end.getTime(),
            );
    ...
    private getTechAvailabilityForWindow(
        ...
        for (const tech of technicians) {
            const conflict = incidents.find(i => {
                if (i.currentTechnicianId?.toString() !== tech.id.toString()) return false;
                // Hay conflicto si el incident se solapa con la ventana
                return i.scheduledRange.start < windowEnd && i.scheduledRange.end > windowStart;
            });
