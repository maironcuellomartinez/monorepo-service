...
import { CompanyId, IssueTypeId, ServiceNowProfileId } from '@app/shared/types/branded-ids';
...
export class ServiceNowIntegrationService {
    ...
    private async resolveSnowCompanySysId(company: Company): Promise<string | null> {
        if (!company.profileId) {
            const fallback = process.env.SN_DEFAULT_COMPANY_SYS_ID ?? null;
            this.logger.debug(`[company:${company.id}] sin perfil SN — usando fallback company_sys_id: ${fallback ?? 'null'}`);
            return fallback;
        }

        const profileResult = await this.profileRepo.findById(company.profileId as ServiceNowProfileId);
        if (profileResult.isFailure || !profileResult.unwrap()) {
            const fallback = process.env.SN_DEFAULT_COMPANY_SYS_ID ?? null;
            this.logger.warn(`[company:${company.id}] perfil ${company.profileId} no encontrado — usando fallback company_sys_id: ${fallback ?? 'null'}`);
            return fallback;
        }

        const sysId = profileResult.unwrap()!.snowCompanySysId.value;
        this.logger.debug(`[company:${company.id}] perfil ${company.profileId} → company_sys_id: ${sysId}`);
        return sysId;
    }
    ...
    private async _createIncidentTicket(incident: Incident, company: Company): Promise<Result<void>> {
        ...
        if (!issueType) return Result.err(new IssueTypeNotFoundError(incident.issueTypeId));
        ...
        if (!corner) return Result.err(new Error(`Corner ${incident.cornerId} not found`));

        const assignmentGroup = await this.resolveAssignmentGroup(company.id, incident.issueTypeId, corner.snowAssignmentGroup ?? null);
        ...
        if (snResult.isFailure) {
            this.logger.error(`[incident:${incident.id}] SN createIncident falló: ${snResult.unwrapError().message}`);
        ...
        if (deferred && correlationId) {
            ...
            this.logger.log(`[incident:${incident.id}] modo ASYNC — correlationId: ${correlationId}`);
        } else {
            ...
            if (sysIdResult.isFailure || numResult.isFailure) {
                this.logger.error(`[incident:${incident.id}] SN devolvió ID/Number inválidos — sysId=${sysId} number=${number}`);
                return Result.err(new Error(`ServiceNow returned invalid sys_id or number: sysId=${sysId} number=${number}`));
            }

            incident.updateServiceNowInfo(sysIdResult.unwrap(), numResult.unwrap());
            this.logger.log(`[incident:${incident.id}] modo INMEDIATO — number: ${number} | sysId: ${sysId}`);
        }
    ...
    private async _createRequestTicket(request: Request, company: Company): Promise<Result<void>> {
        ...
        if (issueTypeResult.isFailure) return Result.err(issueTypeResult.unwrapError());
        const issueType = issueTypeResult.unwrap();
        if (!issueType) return Result.err(new IssueTypeNotFoundError(request.issueTypeId));

        const cornerResult = await this.cornerRepo.findById(request.cornerId);
        if (cornerResult.isFailure) return Result.err(cornerResult.unwrapError());
        const corner = cornerResult.unwrap();
        if (!corner) return Result.err(new Error(`Corner ${request.cornerId} not found`));

        const assignmentGroup = await this.resolveAssignmentGroup(company.id, request.issueTypeId, corner.snowAssignmentGroup ?? null);
        const snowCompanySysId = await this.resolveSnowCompanySysId(company);
        ...
        if (snResult.isFailure) {
            this.logger.error(`[request:${request.id}] SN createRequest falló: ${snResult.unwrapError().message}`);
            return Result.err(snResult.unwrapError());
        }
        ...
        if (deferred && correlationId) {
            ...
            this.logger.log(`[request:${request.id}] modo ASYNC — correlationId: ${correlationId}`);
        } else {
            const sysIdResult = ServiceNowId.create(sysId);
            const numResult = ServiceNowNumber.create(number);

            if (sysIdResult.isFailure || numResult.isFailure) {
                this.logger.error(`[request:${request.id}] SN devolvió ID/Number inválidos — sysId=${sysId} number=${number}`);
                return Result.err(new Error(`ServiceNow returned invalid sys_id or number: sysId=${sysId} number=${number}`));
            }

            request.updateServiceNowInfo(sysIdResult.unwrap(), numResult.unwrap());
            this.logger.log(`[request:${request.id}] modo INMEDIATO — number: ${number} | sysId: ${sysId}`);
        }
    ...
    private async _reQueueIncidentTicket(incident: Incident, company: Company): Promise<Result<void>> {
        ...
        if (enqueueResult.isFailure) {
            this.logger.error(`[incident:${incident.id}] re-enqueue falló: ${enqueueResult.unwrapError().message}`);
            return Result.err(enqueueResult.unwrapError());
        }

        const { correlationId } = enqueueResult.unwrap();
        incident.setSnowqCorrelationId(correlationId);
        this.logger.log(`[incident:${incident.id}] re-enqueued → correlationId: ${correlationId}`);
        return Result.ok(undefined);
    }
    ...
    private async _reQueueRequestTicket(request: Request, company: Company): Promise<Result<void>> {
        ...
        if (enqueueResult.isFailure) {
            this.logger.error(`[request:${request.id}] re-enqueue falló: ${enqueueResult.unwrapError().message}`);
            return Result.err(enqueueResult.unwrapError());
        }

        const { correlationId } = enqueueResult.unwrap();
        request.setSnowqCorrelationId(correlationId);
        this.logger.log(`[request:${request.id}] re-enqueued → correlationId: ${correlationId}`);
        return Result.ok(undefined);
    }
    ...
    private async _closeIncidentTicket(sysId: string, closeCategory: string, closeNotes?: string): Promise<Result<void>> {
        ...
        if (result.isFailure) {
            this.logger.error(`[close] falló para sysId=${sysId}: ${result.unwrapError().message}`);
        }
        return result;
    }
    ...
    private async _updateTicket(table: string, sysId: string, fields: Record<string, any>): Promise<Result<void>> {
        ...
        if (result.isFailure) {
            this.logger.error(`[update] falló para ${table}/${sysId}: ${result.unwrapError().message}`);
        }
        return result;
    }
    ...
    private async resolveAssignmentGroup(
        ...
        const specificResult = await this.companyIssueConfigRepo.getServiceNowGroup(companyId, issueTypeId);
        ...
        if (defaultCompanyId && defaultCompanyId !== companyId.toString()) {
            ...
            const defaultResult = await this.companyIssueConfigRepo.getServiceNowGroup(
        ...
        // 3. Grupo del corner
        if (cornerGroup) {
            this.logger.debug(`[company:${companyId}] usando grupo del corner → ${cornerGroup}`);
            return cornerGroup;
        }
