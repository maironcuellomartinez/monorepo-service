...
import { IncidentId, IssueTypeId, CustomerId, CornerId, SlotId, TechnicianId } from '@app/shared/types/branded-ids';
...
function makeIncident(status?: IncidentStatus): Incident {
    ...
    const toInProgress = () => { toDelivered(); incident.changeStatus(IncidentStatus.IN_PROGRESS, tech); };
    const toClosed = () => { toInProgress(); incident.changeStatus(IncidentStatus.PENDING_PICKUP, tech); incident.changeStatus(IncidentStatus.CLOSED, tech); };
...
function buildMocks(opts?: {
    ...
    const incident = opts?.incident !== undefined ? opts.incident : makeIncident();
    const slots = opts?.slots ?? [makeSlotMock('slot-1')];
    if (opts?.slotsUnavailable) slots.forEach(s => { s.isAvailable.mockReturnValue(false); s.isAvailableForUser.mockReturnValue(false); });
    ...
    const deviceService = {
        resolveAndLinkDevice: jest.fn().mockResolvedValue(Result.ok({
            id: 'device-1',
        })),
    };

    const eventBus = { publishMany: jest.fn().mockResolvedValue(undefined) };
    const cache = { deletePattern: jest.fn().mockResolvedValue(Result.ok(undefined)) };
...
describe('IncidentService.createIncident()', () => {
    ...
    it('invalida el caché con la clave correcta de disponibilidad', async () => {
        ...
        expect(cache.deletePattern).toHaveBeenCalledWith(`availability:corner-1:${dateStr}:*`);
    });
    ...
    it('reserva los slots de forma atómica antes de guardar (F1.4 — bookManyAtomic)', async () => {
        ...
        expect(slotRepository.bookManyAtomic).toHaveBeenCalledWith([SlotId('slot-1')], undefined);
    ...
    it('falla con error claro cuando hay conflicto de slot concurrente (F1.5)', async () => {
        ...
        expect(result.isFailure).toBe(true);
        expect(result.unwrapError().message).toMatch(/horario.*no está disponible|no está disponible/i);
    });
...
describe('IncidentService.takeIncident()', () => {
    it('asigna técnico, guarda y publica evento', async () => {
        ...
        const result = await service.takeIncident({ incidentId: IncidentId('inc-1'), technicianId: TechnicianId('tech-1') });
    ...
    it('falla si la incidencia no existe', async () => {
        ...
        const result = await service.takeIncident({ incidentId: IncidentId('inc-x'), technicianId: TechnicianId('tech-1') });
    ...
    it('falla si el repositorio falla en findById', async () => {
        const { service, incidentRepository } = buildMocks();
        incidentRepository.findById.mockResolvedValue(Result.err(new Error('DB error')));
...
describe('IncidentService.releaseIncident()', () => {
    it('libera técnico asignado sin cambiar estado', async () => {
        ...
        const result = await service.releaseIncident({ incidentId: IncidentId('inc-1'), technicianId: TechnicianId('tech-1'), reason: 'turno' });
    ...
    it('falla si la incidencia no existe', async () => {
        ...
        const result = await service.releaseIncident({ incidentId: IncidentId('x'), technicianId: TechnicianId('tech-1') });
...
describe('IncidentService.changeStatus()', () => {
    ...
    it('transiciona PENDING_PICKUP → CLOSED y registra closedAt', async () => {
        ...
        incident.changeStatus(IncidentStatus.PENDING_PICKUP, TechnicianId('tech-1'));
        const { service, incidentRepository } = buildMocks();
        incidentRepository.findById.mockResolvedValue(Result.ok(incident));
    ...
    it('marca los slots como EXPIRED al cancelar', async () => {
        const incident = makeIncident(IncidentStatus.CREATED);
        const slot = makeSlotMock('slot-1');
        const { service, incidentRepository, slotRepository } = buildMocks();
        incidentRepository.findById.mockResolvedValue(Result.ok(incident));
        slotRepository.findManyByIds.mockResolvedValue(Result.ok([slot]));
        ...
        expect(result.isSuccess).toBe(true);
        expect(slot.expire).toHaveBeenCalledTimes(1);
...
describe('IncidentService.validateIncident()', () => {
    it('transiciona de CLOSED a VALIDATED', async () => {
        ...
        const result = await service.validateIncident({ incidentId: IncidentId('inc-1'), customerId: CustomerId('cust-1') });
...
describe('IncidentService.reopenIncident()', () => {
    it('transiciona de CLOSED a REOPENED e invalida caché', async () => {
        ...
        const result = await service.reopenIncident({ incidentId: IncidentId('inc-1'), customerId: CustomerId('cust-1'), reason: 'sigue fallando' });
    ...
    it('falla si no está en CLOSED', async () => {
        ...
        const result = await service.reopenIncident({ incidentId: IncidentId('inc-1'), customerId: CustomerId('cust-1') });
...
describe('IncidentService — queries', () => {
    it('getIncident() delega al repositorio', async () => {
        ...
        expect(incidentRepository.findById).toHaveBeenCalledWith(IncidentId('inc-1'));
    });
    ...
    it('getAvailableIncidents() delega al repositorio', async () => {
        ...
        expect(incidentRepository.findAvailable).toHaveBeenCalledWith(CornerId('corner-1'));
    });
});
