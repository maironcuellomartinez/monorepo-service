...
export interface IServiceNowClient {
     ...
     */
    enqueueIncident(payload: ServiceNowIncidentPayload): Promise<Result<SnowqCorrelationResult>>;

    /**
     * Encola una request en snowq sin intentar creación inmediata.
     */
    enqueueRequest(payload: ServiceNowRequestPayload): Promise<Result<SnowqCorrelationResult>>;

    /**
     * Consulta el estado de un correlationId en snowq.
     * Retorna null si el correlationId no existe (404).
     */
    getReconcileStatus(correlationId: string): Promise<Result<SnowqStatusResult | null>>;
