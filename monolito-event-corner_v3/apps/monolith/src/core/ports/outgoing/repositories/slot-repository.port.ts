...
export interface ISlotRepository {
    ...
    findByCornerAndDateRange(cornerId: string, dateRange: DateRange): Promise<Result<Slot[]>>;
    findAvailableByCornerAndDateRange(cornerId: string, dateRange: DateRange): Promise<Result<Slot[]>>;
    ...
    findConsecutiveSlots(cornerId: string, startTime: Date, durationMinutes: number): Promise<Result<Slot[]>>;
     ...
     */
    holdManyAtomic(slotIds: SlotId[], userId: string, ttlMinutes: number): Promise<Result<number>>;

    /**
     * Libera los holds del usuario sobre los slots indicados (HELD → AVAILABLE).
     * Solo afecta filas donde held_by_user_id = userId.
     */
    releaseHoldsAtomic(slotIds: SlotId[], userId: string): Promise<Result<number>>;
     ...
     */
    convertHoldsToBooked(slotIds: SlotId[], userId: string): Promise<Result<number>>;
