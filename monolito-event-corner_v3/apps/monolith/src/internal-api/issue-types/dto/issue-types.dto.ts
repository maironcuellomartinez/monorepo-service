...
export class CreateIssueTypeDto {
    @ApiPropertyOptional({ example: 'uuid-tree', description: 'ID del árbol de categorías. Si se omite, se usa el primero disponible.' })
    @IsOptional() @IsString()
    treeId?: string;
...
export class UpdateIssueTypeDto {
    ...
    @ApiPropertyOptional({ enum: IssueCategory }) @IsOptional() @IsString()
    category?: IssueCategory;
