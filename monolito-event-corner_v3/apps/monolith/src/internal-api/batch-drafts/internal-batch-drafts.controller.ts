...
export class InternalBatchDraftsController {
    ...
    async addItem(@Query('userId') userId: string, @Body() body: AddBatchItemCommand) {
        return this.tracing.run('monolith.controller.batchDrafts.addItem', { kind: 'server', attributes: { 'user.id': userId } }, () => this._addItem(userId, body));
    }
    ...
    async editItem(
        ...
        return this.tracing.run('monolith.controller.batchDrafts.editItem', { kind: 'server', attributes: { 'item.id': itemId } }, () => this._editItem(userId, itemId, body));
    }
    ...
    async removeItem(@Query('userId') userId: string, @Param('id') itemId: string) {
        return this.tracing.run('monolith.controller.batchDrafts.removeItem', { kind: 'server', attributes: { 'item.id': itemId } }, () => this._removeItem(userId, itemId));
    }
    ...
    async submit(@Query('userId') userId: string) {
        return this.tracing.run('monolith.controller.batchDrafts.submit', { kind: 'server', attributes: { 'user.id': userId } }, () => this._submit(userId));
    }
    ...
    async discard(@Query('userId') userId: string) {
        return this.tracing.run('monolith.controller.batchDrafts.discard', { kind: 'server', attributes: { 'user.id': userId } }, () => this._discard(userId));
    }
