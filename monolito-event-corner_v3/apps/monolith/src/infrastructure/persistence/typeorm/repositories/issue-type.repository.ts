...
export class TypeOrmIssueTypeRepository implements IIssueTypeRepository {
  ...
  async findById(id: IssueTypeId | string): Promise<Result<IssueType | null>> {
    try {
      const entity = await this.repo.findOne({
        where: { issue_type_id: id.toString() },
      });
  ...
  async findWithFilters(filters: {
    ...
    try {
      ...
      if (filters.visibleToUsers !== undefined) {
        qb.andWhere('it.not_user_visible = :notVisible', {
          notVisible: !filters.visibleToUsers,
        });
      }
      if (filters.treeId) {
    ...
    try {
      ...
      await this.repo.update(
        { issue_type_id: entity.issue_type_id },
        {
          ...
          sn_urgency: entity.sn_urgency,
          sn_impact: entity.sn_impact,
          sn_severity: entity.sn_severity,
  ...
  private toEntity(domain: IssueType): IssueTypeEntity {
    ...
    entity.servicenow_close_category =
      domain.servicenowCloseCategory?.value || null;
    entity.sn_urgency = domain.snUrgency.value;
    entity.sn_impact = domain.snImpact.value;
    entity.sn_severity = domain.snSeverity.value;
  ...
  private toDomain(entity: IssueTypeEntity): IssueType {
    const deviceTypeResult = entity.device_type
      ? DeviceType.create(entity.device_type)
      : null;
    const deviceType = deviceTypeResult?.isSuccess
      ? deviceTypeResult.unwrap()
      : null;

    const servicenowCategoryResult = entity.servicenow_category
      ? ServiceNowCategory.create(entity.servicenow_category)
      : null;
    const servicenowCategory = servicenowCategoryResult?.isSuccess
      ? servicenowCategoryResult.unwrap()
      : null;

    const servicenowCloseCategoryResult = entity.servicenow_close_category
      ? ServiceNowCategory.create(entity.servicenow_close_category)
      : null;
    const servicenowCloseCategory = servicenowCloseCategoryResult?.isSuccess
      ? servicenowCloseCategoryResult.unwrap()
      : null;

    // Defensivo: si la BD trae un valor fuera de rango (p. ej. migración de datos
    // legacy), se cae al default en vez de romper la hidratación.
    const snUrgencyResult = SnUrgency.create(entity.sn_urgency);
    const snUrgency = snUrgencyResult.isSuccess
      ? snUrgencyResult.unwrap()
      : SnUrgency.default();
    const snImpactResult = SnImpact.create(entity.sn_impact);
    const snImpact = snImpactResult.isSuccess
      ? snImpactResult.unwrap()
      : SnImpact.default();
    const snSeverityResult = SnSeverity.create(entity.sn_severity);
    const snSeverity = snSeverityResult.isSuccess
      ? snSeverityResult.unwrap()
      : SnSeverity.default();
    ...
    if (workMinutesResult.isFailure)
      throw new Error(
        `Invalid work_minutes in DB for issue_type ${entity.issue_type_id}: ${entity.work_minutes}`,
      );
    if (spareMinutesResult.isFailure)
      throw new Error(
        `Invalid spare_minutes in DB for issue_type ${entity.issue_type_id}: ${entity.spare_minutes}`,
      );
    if (closeMinutesResult.isFailure)
      throw new Error(
        `Invalid close_minutes in DB for issue_type ${entity.issue_type_id}: ${entity.close_minutes}`,
      );
    if (positionResult.isFailure)
      throw new Error(
        `Invalid position in DB for issue_type ${entity.issue_type_id}: ${entity.position}`,
      );

    return IssueType.reconstitute(
      IssueTypeId(entity.issue_type_id),
      IssueTypeTreeId(entity.tree_id),
      entity.name,
