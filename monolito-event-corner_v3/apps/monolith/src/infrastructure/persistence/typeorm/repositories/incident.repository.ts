...
import { IIncidentRepository, IncidentFilters, IncidentTimelineEntry, PaginatedResult } from '../../../../core/ports/outgoing/repositories/incident-repository.port';
...
import { CornerId, CustomerId, DeviceId, IncidentId, IssueTypeId, LockerId, SlotId, TechnicianId } from '@app/shared/types/branded-ids';

@Injectable()
export class TypeOrmIncidentRepository implements IIncidentRepository {
    constructor(
        @InjectRepository(IncidentEntity) private readonly incidentRepository: Repository<IncidentEntity>,
        @InjectRepository(IncidentSlotEntity) private readonly incidentSlotRepository: Repository<IncidentSlotEntity>,
        @InjectRepository(IncidentTimelineEntity) private readonly timelineRepository: Repository<IncidentTimelineEntity>,
    ) { }
    ...
    async save(incident: Incident): Promise<Result<void>> {
        try {
            ...
            await this.incidentSlotRepository.delete({ incident_id: incident.id.toString() });
    ...
    async findById(id: IncidentId | string): Promise<Result<Incident | null>> {
        try {
            const entity = await this.incidentRepository.findOne({
                ...
                relations: ['issueType', 'customer', 'corner', 'currentTechnician', 'device', 'locker'],
            });
    ...
    async findByStatus(cornerId: string, statuses: IncidentStatus[]): Promise<Result<Incident[]>> {
        try {
    ...
    async findByDateRange(cornerId: string, start: Date, end: Date): Promise<Result<Incident[]>> {
        try {
            const entities = await this.incidentRepository.find({
                where: {
                    corner_id: cornerId,
    ...
    async findWithFilters(filters: IncidentFilters): Promise<Result<PaginatedResult<Incident>>> {
        try {
            ...
            if (filters.cornerId) {
                queryBuilder.andWhere('incident.corner_id = :cornerId', { cornerId: filters.cornerId });
            }

            if (filters.customerId) {
                queryBuilder.andWhere('incident.customer_id = :customerId', { customerId: filters.customerId });
            }

            if (filters.technicianId) {
                queryBuilder.andWhere('incident.current_technician_id = :technicianId', { technicianId: filters.technicianId });
            ...
            if (filters.status && filters.status.length > 0) {
                queryBuilder.andWhere('incident.status IN (:...status)', { status: filters.status });
            }

            if (filters.issueTypeId) {
                queryBuilder.andWhere('incident.issue_type_id = :issueTypeId', { issueTypeId: filters.issueTypeId });
            }

            if (filters.customerEmail) {
                queryBuilder.andWhere('customer.email LIKE :customerEmail', { customerEmail: `%${filters.customerEmail}%` });
            }

            if (filters.servicenowNumber) {
                queryBuilder.andWhere('incident.servicenow_number LIKE :snNumber', { snNumber: `%${filters.servicenowNumber}%` });
            }

            if (filters.deviceSerial) {
                queryBuilder.andWhere('device.serial_number LIKE :deviceSerial', { deviceSerial: `%${filters.deviceSerial}%` });
            }

            if (filters.fromDate) {
                queryBuilder.andWhere('incident.scheduled_start >= :fromDate', { fromDate: filters.fromDate });
            }

            if (filters.toDate) {
                queryBuilder.andWhere('incident.scheduled_start <= :toDate', { toDate: filters.toDate });
            }

            queryBuilder.orderBy('incident.scheduled_start', 'DESC').skip(skip).take(limit);
            ...
            const data = await Promise.all(
                entities.map(async e => {
                    ...
                    return this.toDomain(e, slotRelations.map(r => r.slot_id));
                })
            );
    ...
    async findByServiceNowNumber(number: string): Promise<Result<Incident | null>> {
        try {
            const entity = await this.incidentRepository.findOne({
            ...
            if (!entity) return Result.ok(null);
            const slotRelations = await this.incidentSlotRepository.find({ where: { incident_id: entity.incident_id } });
            const domain = await this.toDomain(entity, slotRelations.map(r => r.slot_id));
            return Result.ok(domain);
        } catch (error) {
            return Result.err(error);
        }
    ...
    async findActiveWithServiceNowId(): Promise<Result<Incident[]>> {
        try {
            ...
            const incidents = await Promise.all(entities.map(e => this.toDomain(e, [])));
    ...
    async findActiveCustomerIds(): Promise<Result<{ customerId: string; externalId: string | null }[]>> {
        try {
            const activeStatuses = [
                IncidentStatus.CREATED,
                IncidentStatus.DELIVERED,
    ...
    async findOrphanedSnowIncidents(minAgeMinutes: number): Promise<Result<Incident[]>> {
        try {
            ...
            const entities = await this.incidentRepository
                ...
                .andWhere('incident.status NOT IN (:...terminalStatuses)', { terminalStatuses })
    ...
    async getTimeline(incidentId: string): Promise<Result<IncidentTimelineEntry[]>> {
        try {
    ...
    async addNote(incidentId: string, technicianId: string | null, comment: string): Promise<Result<void>> {
        try {
    ...
    private async toDomain(entity: IncidentEntity, slotIds: string[]): Promise<Incident> {
        ...
        const incident = Incident.reconstitute(
            IncidentId(entity.incident_id as any),
            ...
            entity.priority,
            entity.current_technician_id ? TechnicianId(entity.current_technician_id as any) : null,
            entity.device_id || null,
            entity.locker_id ? LockerId(entity.locker_id as any) : null,
            servicenowId,
            servicenowNumber,
            entity.metadata || {},
