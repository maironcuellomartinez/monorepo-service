...
export class TypeOrmServiceNowProfileRepository implements IServiceNowProfileRepository {
    ...
    async findById(id: ServiceNowProfileId | string): Promise<Result<ServiceNowProfile | null>> {
        try {
    ...
    async findByCompanySysId(sysId: string): Promise<Result<ServiceNowProfile | null>> {
        try {
            const entity = await this.repo.findOne({
    ...
    private toDomain(entity: ServiceNowProfileEntity): ServiceNowProfile {
        const snowCompanySysIdResult = ServiceNowId.create(entity.snow_company_sys_id);
        if (snowCompanySysIdResult.isFailure) {
            throw new Error(`Invalid ServiceNow ID in database: ${entity.snow_company_sys_id}`);
        }

        return ServiceNowProfile.reconstitute(
            ServiceNowProfileId(entity.profile_id as any),
            entity.name,
