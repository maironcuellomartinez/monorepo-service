...
import { ISlotRepository, SlotFilters } from '../../../../core/ports/outgoing/repositories/slot-repository.port';
...
export class TypeOrmSlotRepository implements ISlotRepository {
    ...
    async saveMany(slots: Slot[]): Promise<Result<void>> {
        ...
        try {
            ...
            // insert() genera una sola query SQL en lugar de N upserts individuales
            const CHUNK = 500;
            for (let i = 0; i < entities.length; i += CHUNK) {
                await this.repo.insert(entities.slice(i, i + CHUNK));
            }
            return Result.ok(undefined);
        } catch (error) {
            return Result.err(error);
    ...
    async findByCornerAndDateRange(cornerId: CornerId, dateRange: DateRange): Promise<Result<Slot[]>> {
        try {
            const entities = await this.repo.find({
                where: {
                    corner_id: cornerId,
    ...
    async findAvailableByCornerAndDateRange(cornerId: CornerId, dateRange: DateRange): Promise<Result<Slot[]>> {
        try {
            const entities = await this.repo.find({
                where: {
                    corner_id: cornerId,
    ...
    async findWithFilters(filters: SlotFilters): Promise<Result<Slot[]>> {
        try {
            const queryBuilder = this.repo.createQueryBuilder('slot')
            ...
            if (filters.status && filters.status.length > 0) {
                queryBuilder.andWhere('slot.status IN (:...status)', { status: filters.status });
            }
    ...
    async findConsecutiveSlots(cornerId: string, startTime: Date, durationMinutes: number): Promise<Result<Slot[]>> {
        try {
            ...
            if (consecutiveSlots.length === 0 ||
                consecutiveSlots[consecutiveSlots.length - 1].ends_at.getTime() < endTime.getTime()) {
                return Result.ok([]);
            }
    ...
    async findByCornerAndDate(cornerId: string, date: Date): Promise<Result<Slot[]>> {
        try {
    ...
    async hasSlotsBySchedule(scheduleId: string): Promise<Result<boolean>> {
        try {
            const count = await this.repo.count({ where: { schedule_id: scheduleId } });
    ...
    async bookManyAtomic(slotIds: SlotId[], userId?: string): Promise<Result<number>> {
        try {
            ...
            const qb = this.repo
                .createQueryBuilder()
                .update()
                .set({ status: SlotStatus.BOOKED, held_by_user_id: null, held_until: null, updated_at: now });

            if (userId) {
                qb.where(
                    'slot_id IN (:...ids) AND (status = :available OR (status = :held AND (held_by_user_id = :userId OR held_until < :now)))',
                    { ids, available: SlotStatus.AVAILABLE, held: SlotStatus.HELD, userId, now },
                );
            } else {
    ...
    async holdManyAtomic(slotIds: SlotId[], userId: string, ttlMinutes: number): Promise<Result<number>> {
        try {
            const ids = slotIds.map((id) => id.toString());
            const now = new Date();
            ...
            const result = await this.repo
                .createQueryBuilder()
                .update()
                .set({ status: SlotStatus.HELD, held_by_user_id: userId, held_until: heldUntil, updated_at: now })
                .where(
    ...
    async releaseHoldsAtomic(slotIds: SlotId[], userId: string): Promise<Result<number>> {
        try {
            const ids = slotIds.map((id) => id.toString());
            const result = await this.repo
                .createQueryBuilder()
                .update()
                .set({ status: SlotStatus.AVAILABLE, held_by_user_id: null, held_until: null, updated_at: new Date() })
                .where(
    ...
    async convertHoldsToBooked(slotIds: SlotId[], userId: string): Promise<Result<number>> {
        try {
            const ids = slotIds.map((id) => id.toString());
            const result = await this.repo
                .createQueryBuilder()
                .update()
                .set({ status: SlotStatus.BOOKED, held_by_user_id: null, held_until: null, updated_at: new Date() })
                .where(
                    'slot_id IN (:...ids) AND status = :held AND held_by_user_id = :userId',
                    { ids, held: SlotStatus.HELD, userId },
                )
