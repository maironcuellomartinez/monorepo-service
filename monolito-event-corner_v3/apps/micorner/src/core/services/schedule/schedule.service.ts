...
} from '../../ports/incoming/schedule/schedule-service.port';
...
export class ScheduleService implements IScheduleService {
  ...
  ): Promise<Result<CornerSchedule>> {
    return this.tracing.run(
      'micorner.createSchedule',
  ...
  private async _createSchedule(
    ...
    // El fallo no puede propagarse al caller (ya respondió) — se loguea con
    // nivel error para que la observabilidad lo capture; el admin puede
    // regenerar editando el schedule.
    this.generateSlotsForSchedule(schedule.id)
      .then((result) => {
        if (result.isFailure) {
          this.logger.error(
            `Error generando slots para schedule ${schedule.id}: ${result.unwrapError().message}`,
          );
        }
      })
      .catch((err) => {
        this.logger.error(
          `Excepción generando slots para schedule ${schedule.id}: ${(err as Error)?.message ?? String(err)}`,
        );
      });
  ...
  private async _updateSchedule(
    ...
    if (affectsSlots) {
      ...
      const hasBooked = await this.slotRepo.hasBookedSlotsBySchedule(
        id.toString(),
      );
      ...
      if (hasBooked.unwrap()) {
        return Result.err(
          new Error(
            'No se puede modificar el horario: tiene turnos ya reservados en incidencias activas',
          ),
        );
      }

      // Eliminar slots no usados para regenerarlos con los nuevos parámetros
      const deleteResult = await this.slotRepo.deleteUnusedBySchedule(
        id.toString(),
      );
    ...
    if (command.isActive !== undefined) {
      if (command.isActive) schedule.activate();
    ...
    if (affectsSlots) {
      this.generateSlotsForSchedule(schedule.id)
        .then((result) => {
          if (result.isFailure) {
            this.logger.error(
              `Error regenerando slots para schedule ${schedule.id}: ${result.unwrapError().message}`,
            );
          } else {
            this.logger.log(
              `Slots regenerados para schedule ${schedule.id}: ${result.unwrap()}`,
            );
          }
        })
        .catch((err) => {
          this.logger.error(
            `Excepción regenerando slots para schedule ${schedule.id}: ${(err as Error)?.message ?? String(err)}`,
          );
        });
    }
  ...
  async assignTechnicians(
    ...
    return this.tracing.run(
      'micorner.assignTechnicians',
      {
        kind: 'server',
        attributes: { 'schedule.scheduleId': `${scheduleId}` },
      },
  ...
  private async _deleteSchedule(id: ScheduleId): Promise<Result<void>> {
    ...
    if (scheduleResult.isFailure)
      return Result.err(scheduleResult.unwrapError());
    const schedule = scheduleResult.unwrap();
    ...
    if (hasBooked.unwrap()) {
      return Result.err(
        new Error(
          'No se puede eliminar un horario que tiene turnos reservados en incidencias activas',
        ),
      );
    }

    // Eliminar slots no usados antes de borrar el schedule
    const deleteSlots = await this.slotRepo.deleteUnusedBySchedule(
      id.toString(),
    );
  ...
  private async _removeTechnician(
    scheduleId: ScheduleId,
    technicianId: TechnicianId,
  ): Promise<Result<void>> {
    const scheduleResult = await this.scheduleRepo.findById(scheduleId);
  ...
  async generateSlotsForSchedule(
    ...
    const corner = cornerResult.unwrap();
    if (!corner)
      return Result.err(new Error(`Corner ${schedule.cornerId} not found`));

    if (!corner.timezone) {
      this.logger.warn(
        `[SlotGen] Corner ${corner.id} no tiene timezone configurado. Los slots se generarán en UTC y se mostrarán desfasados. Configurá el timezone del corner.`,
      );
    }
    ...
    this.logger.log(
      `[SlotGen] Schedule ${scheduleId} | timezone=${timezone} | startTime=${schedule.startTime} | endTime=${schedule.endTime} | validFrom=${schedule.validFrom.toISOString()} | validUntil=${schedule.validUntil.toISOString()}`,
    );
    ...
    this.logger.debug(`[SlotGen] Iterando fechas: ${currentStr} → ${endStr}`);
      ...
      if (schedule.dayOfWeek === null || dayOfWeek === schedule.dayOfWeek) {
        ...
        if (!isHoliday) {
          const dayStart = fromZonedTime(
            `${currentStr}T${startHHMM}:00`,
            timezone,
          );
          const dayEnd = fromZonedTime(`${currentStr}T${endHHMM}:00`, timezone);
          this.logger.debug(
            `[SlotGen] ${currentStr} | dayStart=${dayStart.toISOString()} | dayEnd=${dayEnd.toISOString()}`,
          );

          let slotStart = dayStart;
          while (slotStart < dayEnd) {
            const slotEnd = new Date(
              slotStart.getTime() + schedule.slotDurationMinutes * 60_000,
            );
    ...
    if (slots.length > 0) {
      ...
      // 0 slots con schedule activo suele indicar configuración errónea
      // (rango de fechas pasado, dayOfWeek que no cae en el rango, feriados)
      this.logger.warn(
        `[SlotGen] Schedule ${scheduleId} no produjo ningún slot — revisar validFrom/validUntil/dayOfWeek`,
      );
    }

    this.logger.log(`[SlotGen] Total slots generados: ${slots.length}`);
    return Result.ok(slots.length);
  }

  async getSchedulesByCorner(
    cornerId: CornerId,
  ): Promise<Result<CornerSchedule[]>> {
