...
export class BatchDraftService {
   ...
   * aún en el micorner (serial manual/virtual): en ese caso no hay nada que
  ...
  async addItem(
    userId: string,
    cmd: AddBatchItemCommand,
  ): Promise<Result<BatchDraftItemData>> {
    return this.tracing.run(
      'micorner.batchDraft.addItem',
      {
        kind: 'server',
        attributes: {
          'batchDraft.userId': userId,
          'batchDraft.localId': cmd.localId,
        },
      },
  ...
  private async _addItem(
    ...
    const holdResult = await this.slotRepo.holdManyAtomic(
      slotIds,
      userId,
      HOLD_TTL_MINUTES,
    );
    ...
    if (held < slotIds.length) {
      this.logger.warn(
        `addItem — hold conflict: expected ${slotIds.length}, held ${held} for user ${userId}`,
        CTX,
      );
      return Result.err(
        new Error(
          'El horario seleccionado ya no está disponible. Otro técnico lo reservó.',
        ),
      );
    }
  ...
  async editItem(
    userId: string,
    itemId: string,
    cmd: EditBatchItemCommand,
  ): Promise<Result<void>> {
    return this.tracing.run(
      'micorner.batchDraft.editItem',
      {
        kind: 'server',
        attributes: {
          'batchDraft.userId': userId,
          'batchDraft.itemId': itemId,
        },
      },
  ...
  private async _editItem(
    ...
    // Validar ANTES de tocar los holds: si esto falla después del swap,
    // el item quedaría apuntando a slots liberados y con holds nuevos
    // sin referencia.
    ...
    const slotsChanged =
      cmd.slotIds &&
      JSON.stringify(cmd.slotIds.sort()) !==
        JSON.stringify([...item.slotIds].sort());
  ...
  async removeItem(userId: string, itemId: string): Promise<Result<void>> {
    return this.tracing.run(
      'micorner.batchDraft.removeItem',
      {
        kind: 'server',
        attributes: {
          'batchDraft.userId': userId,
  ...
  private async _submit(
    userId: string,
  ): Promise<Result<BatchSubmitResultItem[]>> {
    const draftResult = await this.repo.findByUserId(userId);
    if (draftResult.isFailure) return Result.err(draftResult.unwrapError());
    const draft = draftResult.unwrap();
    ...
    for (const item of draft.items.filter(
      ...
      if (item.startTime <= now) {
        await this.repo.updateItem(item.id, {
          status: 'error',
          lastError:
            'El horario ya pasó. Editá la incidencia y elegí un nuevo horario.',
        });
        results.push({
          localId: item.localId,
          status: 'error',
          error: 'El horario ya pasó',
        });
        continue;
      }
  ...
  private async _discard(userId: string): Promise<Result<void>> {
    ...
    const allSlotIds = draft.items.flatMap((i) =>
      i.slotIds.map((id) => SlotId(id as any)),
    );
  ...
  async renewHolds(
    userId: string,
  ): Promise<Result<{ renewedUntil: Date; count: number }>> {
    const draftResult = await this.repo.findByUserId(userId);
    if (draftResult.isFailure) return Result.err(draftResult.unwrapError());
    const draft = draftResult.unwrap();
    if (!draft || draft.items.length === 0) {
    ...
    // renewHoldsAtomic (no holdManyAtomic): debe extender held_until de los
    // holds propios todavia vigentes — holdManyAtomic los excluye y la
    // renovacion era un no-op hasta que el hold expiraba.
    const holdResult = await this.slotRepo.renewHoldsAtomic(
      allSlotIds,
      userId,
      HOLD_TTL_MINUTES,
    );
