...
import {
  ...
  IIssueTypeService,
  UpdateIssueTypeCommand,
} from '../../ports/incoming/admin/issue-type-service.port';
import {
  IIssueTypeRepository,
  ISSUE_TYPE_REPOSITORY,
} from '../../ports/outgoing/repositories/issue-type-repository.port';
import {
  IIssueTypeTreeRepository,
  ISSUE_TYPE_TREE_REPOSITORY,
} from '../../ports/outgoing/repositories/issue-type-tree-repository.port';
...
import { SnUrgency } from '@app/core/domain/value-objects/sn-urgency.value';
import { SnImpact } from '@app/core/domain/value-objects/sn-impact.value';
import { SnSeverity } from '@app/core/domain/value-objects/sn-severity.value';
import {
  IssueTypeNotFoundError,
  IssueTypeTreeNotFoundError,
} from '@app/shared/errors/domain-error';
import { TracingService } from '@app/observability';

@Injectable()
export class IssueTypeService implements IIssueTypeService {
  constructor(
    @Inject(ISSUE_TYPE_REPOSITORY)
    private readonly issueTypeRepo: IIssueTypeRepository,
    @Inject(ISSUE_TYPE_TREE_REPOSITORY)
  ...
  private async _createIssueType(
    ...
    if (!resolvedTreeId) {
      ...
      if (!first)
        return Result.err(
          new Error('No hay ningún árbol de tipos de cita configurado'),
        );
      resolvedTreeId = first.id;
    } else {
      ...
      if (!treeResult.unwrap())
        return Result.err(new IssueTypeTreeNotFoundError(resolvedTreeId));
    }
    ...
    const servicenowCategory = command.servicenowCategory
      ...
      : Result.ok(null);
    if (servicenowCategory.isFailure)
      return Result.err(servicenowCategory.unwrapError());

    const servicenowCloseCategory = command.servicenowCloseCategory
      ? ServiceNowCategory.create(command.servicenowCloseCategory)
      : Result.ok(null);
    if (servicenowCloseCategory.isFailure)
      return Result.err(servicenowCloseCategory.unwrapError());

    const snUrgency =
      command.snUrgency !== undefined
        ? SnUrgency.create(command.snUrgency)
        : Result.ok(SnUrgency.default());
    if (snUrgency.isFailure) return Result.err(snUrgency.unwrapError());

    const snImpact =
      command.snImpact !== undefined
        ? SnImpact.create(command.snImpact)
        : Result.ok(SnImpact.default());
    if (snImpact.isFailure) return Result.err(snImpact.unwrapError());

    const snSeverity =
      command.snSeverity !== undefined
        ? SnSeverity.create(command.snSeverity)
        : Result.ok(SnSeverity.default());
    if (snSeverity.isFailure) return Result.err(snSeverity.unwrapError());

    // 3. Crear entidad
    const issueTypeId = crypto.randomUUID() as unknown as IssueTypeId;
    const issueTypeResult = IssueType.create(
      issueTypeId,
      IssueTypeTreeId(resolvedTreeId),
      command.name,
      command.category,
      deviceType.unwrap(),
      workMinutes.unwrap(),
  ...
  private async _updateIssueType(
    ...
    const deviceType =
      command.deviceType !== undefined
        ? command.deviceType
          ? DeviceType.create(command.deviceType)
          : Result.ok(null)
        : null;
    if (deviceType && deviceType.isFailure)
      return Result.err(deviceType.unwrapError());

    const position =
      command.position !== undefined ? Position.create(command.position) : null;
    if (position && position.isFailure)
      return Result.err(position.unwrapError());

    const workMinutes =
      command.workMinutes !== undefined
        ? WorkMinutes.create(command.workMinutes)
        : null;
    if (workMinutes && workMinutes.isFailure)
      return Result.err(workMinutes.unwrapError());

    const spareMinutes =
      command.spareMinutes !== undefined
        ? SpareMinutes.create(command.spareMinutes)
        : null;
    if (spareMinutes && spareMinutes.isFailure)
      return Result.err(spareMinutes.unwrapError());

    const closeMinutes =
      command.closeMinutes !== undefined
        ? CloseMinutes.create(command.closeMinutes)
        : null;
    if (closeMinutes && closeMinutes.isFailure)
      return Result.err(closeMinutes.unwrapError());

    const servicenowCategory =
      command.servicenowCategory !== undefined
        ? command.servicenowCategory
          ? ServiceNowCategory.create(command.servicenowCategory)
          : Result.ok(null)
        : null;
    if (servicenowCategory && servicenowCategory.isFailure)
      return Result.err(servicenowCategory.unwrapError());

    const servicenowCloseCategory =
      command.servicenowCloseCategory !== undefined
        ? command.servicenowCloseCategory
          ? ServiceNowCategory.create(command.servicenowCloseCategory)
          : Result.ok(null)
        : null;
    if (servicenowCloseCategory && servicenowCloseCategory.isFailure)
      return Result.err(servicenowCloseCategory.unwrapError());
    ...
    const snUrgency =
      command.snUrgency !== undefined
        ? SnUrgency.create(command.snUrgency)
        : null;
    if (snUrgency && snUrgency.isFailure)
      return Result.err(snUrgency.unwrapError());

    const snImpact =
      command.snImpact !== undefined ? SnImpact.create(command.snImpact) : null;
    if (snImpact && snImpact.isFailure)
      return Result.err(snImpact.unwrapError());

    const snSeverity =
      command.snSeverity !== undefined
        ? SnSeverity.create(command.snSeverity)
        : null;
    if (snSeverity && snSeverity.isFailure)
      return Result.err(snSeverity.unwrapError());
    ...
    existing.update(
      ...
      deviceType !== null
        ? (deviceType?.unwrap() ?? null)
        : existing.deviceType,
      servicenowCategory !== null
        ? (servicenowCategory?.unwrap() ?? null)
        : existing.servicenowCategory,
      servicenowCloseCategory !== null
        ? (servicenowCloseCategory?.unwrap() ?? null)
        : existing.servicenowCloseCategory,
      workMinutes !== null
        ? (workMinutes?.unwrap() ?? existing.workMinutes)
        : existing.workMinutes,
      spareMinutes !== null
        ? (spareMinutes?.unwrap() ?? existing.spareMinutes)
        : existing.spareMinutes,
      closeMinutes !== null
        ? (closeMinutes?.unwrap() ?? existing.closeMinutes)
        : existing.closeMinutes,
      command.notUserVisible ?? existing.notUserVisible,
      position !== null
        ? (position?.unwrap() ?? existing.position)
        : existing.position,
      command.npsDisabled ?? existing.npsDisabled,
      snUrgency ? snUrgency.unwrap() : undefined,
      snImpact ? snImpact.unwrap() : undefined,
      snSeverity ? snSeverity.unwrap() : undefined,
    );
    ...
    // 5. Publicar evento
    await this.eventBus.publish(
      new DomainEvent('ISSUE_TYPE_UPDATED', command.id, 'IssueType', {
        id: command.id,
      }),
    );
  ...
  private async _deleteIssueType(id: string): Promise<Result<void>> {
    ...
    // 4. Publicar evento
    await this.eventBus.publish(
      new DomainEvent('ISSUE_TYPE_DELETED', id, 'IssueType', { id }),
    );
  ...
  async listIssueTypes(filters?: {
    ...
    if (filters?.visibleToUsers !== undefined) {
      types = types.filter(
        (t) => t.isVisibleToUser() === filters.visibleToUsers,
      );
    }
