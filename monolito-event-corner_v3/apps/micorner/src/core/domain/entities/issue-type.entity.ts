...
import { SnUrgency } from '../value-objects/sn-urgency.value';
import { SnImpact } from '../value-objects/sn-impact.value';
import { SnSeverity } from '../value-objects/sn-severity.value';
...
export class IssueType {
   ...
   */
  get servicenowCategory(): ServiceNowCategory | null {
    return this.props.servicenowCategory;
  }
  /**
   * Categoría de cierre de ServiceNow
   * @returns Categoría de cierre de ServiceNow
   */
  get servicenowCloseCategory(): ServiceNowCategory | null {
    return this.props.servicenowCloseCategory;
  }
  /**
   * Urgencia para ServiceNow (campo `urgency`)
   * @returns Urgencia del ticket en ServiceNow
   */
  get snUrgency(): SnUrgency {
    return this.props.snUrgency;
  }
  /**
   * Impacto para ServiceNow (campo `impact`)
   * @returns Impacto del ticket en ServiceNow
   */
  get snImpact(): SnImpact {
    return this.props.snImpact;
  }
  /**
   * Severidad para ServiceNow (campo `u_severity`)
   * @returns Severidad del ticket en ServiceNow
   */
  get snSeverity(): SnSeverity {
    return this.props.snSeverity;
  }
  /**
   * Minutos de trabajo
   * @returns Minutos de trabajo
   */
  get workMinutes(): WorkMinutes {
    return this.props.workMinutes;
  }
  /**
   * Minutos de repuesto
   * @returns Minutos de repuesto
   */
  get spareMinutes(): SpareMinutes {
    return this.props.spareMinutes;
  }
  /**
   * Minutos de cierre
   * @returns Minutos de cierre
   */
  get closeMinutes(): CloseMinutes {
    return this.props.closeMinutes;
  }
  /**
   ...
   */
  get npsDisabled(): boolean {
    return this.props.npsDisabled;
  }
  /**
  ...
  update(
    ...
    // Opcionales: si el caller no los envía, se conserva el valor actual.
    this.props.snUrgency = snUrgency ?? this.props.snUrgency;
    this.props.snImpact = snImpact ?? this.props.snImpact;
    this.props.snSeverity = snSeverity ?? this.props.snSeverity;
  ...
  static create(
    ...
    const issueType = new IssueType({
      ...
      snUrgency: snUrgency ?? SnUrgency.default(),
      snImpact: snImpact ?? SnImpact.default(),
      snSeverity: snSeverity ?? SnSeverity.default(),
      workMinutes,
      spareMinutes,
      closeMinutes,
      notUserVisible,
