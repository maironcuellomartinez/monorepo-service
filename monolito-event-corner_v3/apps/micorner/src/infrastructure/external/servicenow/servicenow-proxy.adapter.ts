...
export class ServiceNowProxyAdapter implements IServiceNowClient {
  ...
  async createIncident(
    ...
    return this.tracing.run(
      'micorner.proxy.sn.createIncident',
      { kind: 'client' },
      () => this._createIncident(payload),
    );
  }
  ...
  private async _createIncident(
    ...
    try {
      const { data } = await firstValueFrom(
        this.http.post<ServiceNowTicketResult>(
  ...
  async createRequest(
    ...
    return this.tracing.run(
      'micorner.proxy.sn.createRequest',
      { kind: 'client' },
  ...
  async enqueueIncident(
    payload: ServiceNowIncidentPayload,
  ): Promise<Result<SnowqCorrelationResult>> {
    return this.tracing.run(
      'micorner.proxy.sn.enqueueIncident',
      { kind: 'client' },
      () => this._enqueueIncident(payload),
    );
  }

  private async _enqueueIncident(
    payload: ServiceNowIncidentPayload,
  ): Promise<Result<SnowqCorrelationResult>> {
    try {
      const { data } = await firstValueFrom(
        this.http.post<SnowqCorrelationResult>(
          `${this.baseUrl}/incidents/enqueue`,
          payload,
          { headers: this.headers },
        ),
      );
  ...
  async enqueueRequest(
    ...
    return this.tracing.run(
      'micorner.proxy.sn.enqueueRequest',
      { kind: 'client' },
      () => this._enqueueRequest(payload),
    );
  }
  ...
  async getReconcileStatus(
    ...
    return this.tracing.run(
      'micorner.proxy.sn.getReconcileStatus',
      { kind: 'client', attributes: { 'sn.correlationId': correlationId } },
      () => this._getReconcileStatus(correlationId),
    );
  }

  private async _getReconcileStatus(
    correlationId: string,
  ): Promise<Result<SnowqStatusResult | null>> {
    try {
      const { data } = await firstValueFrom(
        this.http.get<SnowqStatusResult | null>(
          `${this.baseUrl}/reconcile/${correlationId}`,
          { headers: this.headers },
        ),
      );
  ...
  async retrySnowqEntry(correlationId: string): Promise<Result<void>> {
    return this.tracing.run(
      'micorner.proxy.sn.retrySnowqEntry',
      { kind: 'client', attributes: { 'sn.correlationId': correlationId } },
  ...
  private async _retrySnowqEntry(correlationId: string): Promise<Result<void>> {
    try {
      await firstValueFrom(
        this.http.post(
          `${this.baseUrl}/reconcile/${correlationId}/retry`,
          {},
          { headers: this.headers },
        ),
      );
  ...
  async updateTicket(
    ...
    return this.tracing.run(
      'micorner.proxy.sn.updateTicket',
  ...
  private async _updateTicket(
    ...
    try {
      await firstValueFrom(
        this.http.patch(`${this.baseUrl}/${table}/${sysId}`, fields, {
          headers: this.headers,
        }),
      );
      return Result.ok(undefined);
  ...
  > {
    return this.tracing.run(
      'micorner.proxy.sn.getCompanies',
      { kind: 'client' },
      () => this._getCompanies(),
    );
  }
  ...
  > {
    try {
      const { data } = await firstValueFrom(
        this.http.get<Array<{ sys_id: string; name: string }>>(
      ...
      return this.handleError('getCompanies', err);
    }
  }
  ...
  private handleError(method: string, err: unknown): Result<never> {
    ...
    const gwMessage: string | undefined =
