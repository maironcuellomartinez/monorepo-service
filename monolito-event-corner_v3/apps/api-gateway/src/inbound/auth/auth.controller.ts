...
class DevLoginDto {
  ...
  @ApiPropertyOptional({
    example: 'dev-user-001',
    description: 'Azure AD OID simulado. Default: hash del email.',
  })
  @IsString()
  @IsOptional()
  oid?: string;
...
export class AuthController {
  ...
  async devLogin(@Body() body: DevLoginDto) {
    return this.tracing.run(
      'gateway.controller.auth.devLogin',
      { kind: 'server' },
      () => this._devLogin(body),
    );
  }
  private async _devLogin(body: DevLoginDto) {
    if (process.env.NODE_ENV !== 'development') {
      throw new ForbiddenException(
        'Este endpoint solo está disponible en entorno development',
      );
    }
    ...
    const oid =
      body.oid ??
      `dev-${Buffer.from(body.email).toString('base64url').slice(0, 16)}`;
