...
export class IncidentsController {
  ...
  @ApiOperation({
    summary: 'Listar usuarios activos — para el picker al crear incidencias',
  })
  async listUsers() {
  ...
  @Roles('technician', 'admin', 'super-admin')
  @ApiOperation({
    summary: 'Buscar usuarios activos — para el picker al crear incidencias',
  })
  @ApiQuery({
    name: 'q',
    required: true,
  ...
  async searchUsers(@Query('q') q: string) {
    // Sin requireCompany: el picker debe encontrar a cualquier usuario activo
    // a medida que se escribe. La UI ya bloquea "Continuar" si el usuario
    // elegido no tiene empresa asignada (no puede crear incidencias).
  ...
  @ApiQuery({
    name: 'availableOnly',
    required: false,
    description: 'true = solo incidencias sin técnico asignado',
  })
  async list(
  ...
  @ApiOperation({
    summary: 'Incidencias del usuario autenticado',
    description:
      'Retorna las incidencias del usuario identificado por el JWT (empleado).',
  })
  ...
  @Permission('incident', 'list')
  @ApiOperation({
    summary: 'Incidencias disponibles para tomar',
    description:
      'Lista incidencias sin técnico asignado en el corner indicado.',
  })
  ...
  async deliver(
    ...
    return this.tracing.run(
      'gateway.controller.incidents.deliver',
  ...
  async changeStatus(
    ...
    return this.tracing.run(
      'gateway.controller.incidents.changeStatus',
      { kind: 'server', attributes: { 'incident.id': id } },
  ...
  @ApiOperation({
    ...
    description:
      'El cliente cancela su incidencia. Solo válido desde CREATED → CANCELED.',
  })
  @ApiParam({ name: 'id' })
  async cancel(
    @Param('id') id: string,
    ...
    return this.tracing.run(
      'gateway.controller.incidents.cancel',
      { kind: 'server', attributes: { 'incident.id': id } },
  ...
  @ApiOperation({
    ...
    description:
      'El cliente confirma que la incidencia está resuelta. Transición CLOSED → VALIDATED.',
  })
  @ApiParam({ name: 'id' })
  async validate(
    @Param('id') id: string,
    ...
    return this.tracing.run(
      'gateway.controller.incidents.validate',
      { kind: 'server', attributes: { 'incident.id': id } },
  ...
  async reopen(
    ...
    return this.tracing.run(
      'gateway.controller.incidents.reopen',
      { kind: 'server', attributes: { 'incident.id': id } },
  ...
  @Permission('incident', 'read')
  @ApiOperation({
    summary: 'Historial de la incidencia',
    description:
      'Devuelve la línea de tiempo completa de la incidencia ordenada cronológicamente.',
  })
  @ApiParam({ name: 'id' })
  ...
  @ApiOperation({
    ...
    description: 'Registra una nota sin cambiar el estado de la incidencia.',
  })
  @ApiParam({ name: 'id' })
  async addNote(
    @Param('id') id: string,
    ...
    return this.tracing.run(
      'gateway.controller.incidents.addNote',
      { kind: 'server', attributes: { 'incident.id': id } },
