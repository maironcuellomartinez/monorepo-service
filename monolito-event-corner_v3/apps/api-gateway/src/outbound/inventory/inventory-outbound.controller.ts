...
export class InventoryOutboundController {
    ...
    constructor(
        ...
        this.inventoryUrl = this.config.get<string>('EXTERNAL_INVENTORY_URL', 'http://localhost:3008/api/v1/minerva');
    }
        ...
        try {
            ...
            return {
                ...
                assigned_user: data.assignedTo
                    ? { userId: data.assignedTo, nombre: data.assignedUserName ?? data.assignedTo }
                    : null,
            };
        } catch (err) {
            ...
            this.logger.error(`Inventory proxy getBySerial(${serialNumber}): ${(err as Error).message}`);
            throw new HttpException('Inventory service unavailable', HttpStatus.BAD_GATEWAY);
        }
    }
    ...
    async getByUser(@Param('userId') userId: string) {
        try {
            ...
            this.logger.error(`Inventory proxy getByUser(${userId}): ${(err as Error).message}`);
            throw new HttpException('Inventory service unavailable', HttpStatus.BAD_GATEWAY);
        }
    }
}
