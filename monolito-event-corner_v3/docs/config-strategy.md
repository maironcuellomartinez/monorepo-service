...
| Inventory URL             | `INTEGRATION_SERVICE_INVENTORY_URL`       |
...
### 🆕 Necesita añadirse al `.env` del micorner
...
| `SN_EXTERNAL_ID_FIELD` | ✅ Implementado | `api-snowq-service/src/common/enum/request-type.enum.ts` (`buildPayloadForServiceNow`) — mapea `payload.externalId` al campo configurable (default `u_external_system_id`). El micorner arma `externalId` como `{incidentId}_{corner.code}` en `servicenow-integration.service.ts` (`buildExternalId`). Verificado end-to-end 2026-07-23. |
